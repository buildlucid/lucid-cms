...
export const initializeOAuth = () => {
	const transactionKey = "lucid-integration-playground.transaction";
	const baseUrlKey = "lucid-integration-playground.base-url";
	const transactionLifetimeMs = 6 * 60 * 1000;

	const ui = {
		serverState: element("server-state", HTMLDivElement),
		serverStateLabel: element("server-state-label", HTMLSpanElement),
		lucidUrl: element("lucid-url", HTMLInputElement),
		discoverButton: element("discover-button", HTMLButtonElement),
		connectButton: element("connect-button", HTMLButtonElement),
		readScopesButton: element("read-scopes-button", HTMLButtonElement),
		scopeList: element("scope-list", HTMLDivElement),
		issuer: element("issuer-value", HTMLElement),
		resource: element("resource-value", HTMLElement),
		clientId: element("client-id-value", HTMLElement),
		redirect: element("redirect-value", HTMLElement),
		notice: element("notice", HTMLDivElement),
		noticeTitle: element("notice-title", HTMLElement),
		noticeCopy: element("notice-copy", HTMLElement),
		noticeClose: element("notice-close", HTMLButtonElement),
		flowDiscover: element("flow-discover", HTMLDivElement),
		flowAuthorize: element("flow-authorize", HTMLDivElement),
		flowRequest: element("flow-request", HTMLDivElement),
		tokenEmpty: element("token-empty", HTMLDivElement),
		tokenContent: element("token-content", HTMLDivElement),
		tokenStatus: element("token-status", HTMLSpanElement),
		principalIcon: element("principal-icon", HTMLDivElement),
		principalValue: element("principal-value", HTMLElement),
		grantValue: element("grant-value", HTMLElement),
		expiresValue: element("expires-value", HTMLElement),
		accessToken: element("access-token-value", HTMLElement),
		refreshToken: element("refresh-token-value", HTMLElement),
		grantedScopes: element("granted-scopes", HTMLDivElement),
		revealTokenButton: element("reveal-token-button", HTMLButtonElement),
		refreshButton: element("refresh-button", HTMLButtonElement),
		revokeButton: element("revoke-button", HTMLButtonElement),
		clearButton: element("clear-button", HTMLButtonElement),
		oauthTab: element("oauth-tab", HTMLButtonElement),
		apiKeyTab: element("api-key-tab", HTMLButtonElement),
		apiKeyField: element("api-key-field", HTMLDivElement),
		apiKey: element("api-key-value", HTMLInputElement),
		requestMethod: element("request-method", HTMLSelectElement),
		requestPath: element("request-path", HTMLInputElement),
		bodyField: element("body-field", HTMLLabelElement),
		requestBody: element("request-body", HTMLTextAreaElement),
		requestAuthCaption: element("request-auth-caption", HTMLElement),
		sendButton: element("send-button", HTMLButtonElement),
		responseShell: element("oauth-response", HTMLDivElement),
		responseStatus: element("response-status", HTMLElement),
		responseTime: element("response-time", HTMLElement),
		responseValue: element("response-value", HTMLElement),
		activityList: element("activity-list", HTMLOListElement),
		clearActivityButton: element("clear-activity-button", HTMLButtonElement),
	};
	...
	const readTransaction = (): OAuthTransaction => {
		...
		if (!stored)
			throw new Error("The OAuth transaction is missing or expired.");
	...
	const renderTokens = () => {
		...
		ui.tokenStatus.textContent = active
			? "Active in memory"
			: "No active grant";
	...
	ui.requestMethod.addEventListener("change", () => {
		ui.bodyField.classList.toggle(
			"is-hidden",
			ui.requestMethod.value === "GET",
		);
	});
