...
const html = generateHTML(document.fields.body, {
  ...
  routes: pagesCollection.contentRoutes,
  locale: "en",
  renderers: {
...
- Document links store only a route key, collection key, and document ID. The current URL is resolved from the hydrated document ref and registered route when rendering. If either is unavailable, only the link text is rendered.
...
- `links.internal` is `true` for every registered content route, or an array of allowed collection keys.
...
Document links require a route in `lucid.config`. Plugin recipes can append to the same array:
...
contentRoutes: [
  ...
  },
  {
    key: "posts",
    collectionKey: "posts",
    path: { field: "slug", prefix: "/blog" },
    label: { fields: ["title", "publishedAt"] },
  },
];
```

Route keys are stable identities. A link does not store a fallback path, so a missing document, route, or path value renders as plain text. A custom document-link renderer receives the resolved current `href`.
