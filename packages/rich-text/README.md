...
The default image renderer uses the original file URL for `<img src>` and does not include configured media presets. When available, its base64 preview and average colour are applied as background styles. The marker `data-lucid-rich-text-image-placeholder` is present when a placeholder is applied, so client JavaScript can remove that background after the real image loads. Video posters use the original poster URL and the equivalent `data-lucid-rich-text-poster-placeholder` marker. SVGs remain external image sources and are never inlined.
