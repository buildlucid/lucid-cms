...
const getDefaultLibSQLConfig = (
	...
	if (typeof env.LIBSQL_URL !== "string" || env.LIBSQL_URL.length === 0) {
		throw new Error(
			"LibSQL database adapter requires LIBSQL_URL when using `db: libsql` or `db: libsql()`. Set LIBSQL_URL, or pass explicit options with `libsql({ url, authToken })` or `libsql((env) => ({ ... }))`.",
		);
	}
