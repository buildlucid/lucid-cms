...
import { formatFilterSubject } from "./utils/filter-fields";
...
const FilterPanel: Component<FilterPanelProps> = (props) => {
	...
	const singleMode = createMemo(
		() => props.queryState.orFilterGroups().length === 0,
	);
	...
	const topConditions = createMemo<OrFilterCondition[]>(() => {
		...
		for (const [key, state] of props.queryState.filterStates()) {
	...
	const rows = createMemo<RowModel[]>(() => {
		...
		props.queryState.orFilterGroups().forEach((group, groupIndex) => {
	...
	const removeOrCondition = (groupIndex: number, conditionIndex: number) => {
		...
		props.queryState.orFilterGroups().forEach((g, gi) => {
			g.forEach((_, ci) => {
				let nextGi = gi;
				let nextCi = ci;
	...
	const updateOrCondition = (
		...
		props.queryState.setOrFilterGroups(groups);
	};
		...
		for (const condition of topConditions()) {
			if (isFilterValueEmpty(condition.value)) {
				...
				demotedDrafts.push({
					...
					target: 0,
					key: condition.key,
					operator: condition.operator as FilterOperator | undefined,
					value: "",
				});
	...
	const handleOperatorChange = (row: RowModel, operator: FilterOperator) => {
		if (row.ref.source === "draft") {
			updateDraft(row.ref.draftId, { operator });
			return;
		}
		if (row.ref.source === "top") {
			props.queryState.setFilter(row.ref.key, { value: row.value, operator });
			return;
		}
	...
	const commitDraftValue = (draftId: number, value: FilterValue) => {
		...
		if (singleMode()) {
			...
			batch(() => {
				props.queryState.setFilter(condition.key, {
					value: condition.value,
					operator: condition.operator,
				});
		...
		batch(() => {
			if (draft.target === "new" || draft.target >= groups.length) {
				props.queryState.setOrFilterGroups([...groups, [condition]]);
	...
	const handleValueCommit = (row: RowModel, value: FilterValue) => {
		...
		if (row.ref.source === "top") {
			props.queryState.setFilter(row.ref.key, {
				value,
				operator: row.operator,
			});
			return;
	...
	const handleRemove = (row: RowModel) => {
		...
		batch(() => {
			if (row.ref.source === "draft") {
				...
				props.queryState.clearFilter(row.ref.key);
				removeIdentity(identity(row.ref));
			} else {
				removeOrCondition(row.ref.groupIndex, row.ref.conditionIndex);
			}
		});
		if (isLastRow) props.onOpenChange(false);
	};
	const isPresetActive = (preset: FilterPreset) => {
		return isFilterPresetActive({
			currentFilters: props.queryState.filterStates(),
			orFilterGroups: props.queryState.orFilterGroups(),
	...
	createEffect(() => {
		if (!props.queryState.ready()) return;
		const groups = props.queryState.orFilterGroups();
						...
						{(row, index) => (
							<>
								...
								<FilterRow
									...
									operator={row().operator as FilterOperator | undefined}
									value={row().value}
