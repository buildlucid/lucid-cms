...
import type { MediaImageGenerationTarget } from "@/store/aiModalsStore/aiModalsStore";
...
const CreateUpdateMediaDrawer: Component<CreateUpdateMediaPanelProps> = (
	props,
) => {
	// ------------------------------
	// State & Hooks
	...
	const [posterAlt, setPosterAlt] = createSignal<MediaTranslation[]>([]);
	...
	const existingPoster = createMemo(() => {
		const item = media.data?.data;
		return item?.type === "video" ? item.poster : null;
	});
	const updatePosterMedia = props.id
		? useUpdateMedia(() => existingPoster()?.id)
		: null;
	...
	const showPosterAltInput = createMemo(() => {
		return (
			...
			(PosterFile.getFile() !== null || existingPoster() != null)
		);
	});
	...
	const posterEmptyDescription = createMemo(() => {
		if (PosterFile.getRemovedCurrent() && existingPoster()) {
	...
	const updateData = createMemo(() => {
		...
		const { changed, data } = helpers.updateData(
			...
			},
			{
				key: state?.key(),
				title: state?.title(),
				alt: showAltInput() ? state?.alt() : undefined,
	...
	async function createPoster() {
		...
		const poster = existingPoster();
		if (poster && updatePosterMedia) {
			updatePosterMedia.setTitle([]);
			...
			return success ? poster.id : null;
		}

		if (!file) return null;
		const createdPoster = await createPosterMedia.createMedia(file, imageMeta, {
			title: [],
			alt: posterAlt(),
			folderId: null,
								...
								<div class="w-full rounded-md border border-border bg-input-base mb-3 overflow-hidden relative">
									<div class="grid grid-cols-[112px_1fr_auto] items-center gap-3 p-3">
										...
										<div class="flex items-center justify-center gap-1 self-center">
											...
											<Show
												when={
													PosterFile.getRemovedCurrent() && existingPoster()
												}
											>
												<Button
													type="button"
...
export default CreateUpdateMediaDrawer;
