...
import { getStageColor } from "@/utils/document-sidebar";
import helpers from "@/utils/helpers";
import WorkflowStageOption from "./WorkflowStageOption";
	...
	const assigneeOptions = createMemo(
		() =>
			assigneeQuery.data?.data.map((user) => ({
				value: user.id,
				label: helpers.formatUserName(user, "name") || T()("common.unknown"),
				user,
			})) ?? [],
	);
		...
		setAssignees(
			workflow()?.assignees.map((assignee) => {
				...
				return {
					...
					label: helpers.formatUserName(user, "name") || `#${assignee.userId}`,
					user,
				};
			}) ?? [],
		);
