...
const getDefaultPostgresConfig = (
	...
	if (typeof env.DATABASE_URL !== "string" || env.DATABASE_URL.length === 0) {
		throw new Error(
			"Postgres database adapter requires DATABASE_URL when using `db: postgres` or `db: postgres()`. Set DATABASE_URL, or pass explicit options with `postgres({ url })` or `postgres((env) => ({ ... }))`.",
		);
	}
