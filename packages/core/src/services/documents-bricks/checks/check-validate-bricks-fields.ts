...
> = async (context, data) => {
	...
	if (brickErrors.length > 0 || fieldErrors.length > 0) {
		return {
			...
			error: {
				type: "basic",
				name: text.server("core.fields.validation.error.name"),
				message: text.server("core.fields.validation.error.message"),
				status: 400,
				errors: {
					bricks: brickErrors,
					fields: fieldErrors,
	...
	props.instance.fields.forEach((fieldInstance, key) => {
		...
		if (
			...
			errors.push({
				key: key,
				localeCode: null,
				message: text.server("core.fields.validation.is.required"),
			});
		}
	});
	...
	const buildFieldErrors = (
		...
		if (validationResult.errors?.length) {
			return validationResult.errors.map((error) => ({
				...
				localeCode,
				message:
					error.message || text.server("core.fields.validation.errors.unknown"),
