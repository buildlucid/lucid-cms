...
describe("toSafeTableName", () => {
	test("returns original name when under limit", () => {
		const raw = "lucid_doc__page__banner__rep__items";
	...
	test("hashes only the tail after the stable prefix when over limit", () => {
		const raw =
			"lucid_doc__page__banner__rep__items__nested_items__deep_nested__deeply_nested_items";
		const res = toSafeTableName(raw, 63);

		expect(res.rawName).toBe(raw);
		expect(res.name.startsWith("lucid_doc__page__banner__rep_")).toBe(true);
		expect(res.name).toMatch(/^lucid_doc__page__banner__rep_[0-9a-f]{8}$/);
	});
	...
	test("preserves separator section for other relation table types", () => {
		...
		expect(media.name).toMatch(/^lucid_doc__page__banner__med_[0-9a-f]{8}$/);
		expect(user.name).toMatch(/^lucid_doc__page__banner__usr_[0-9a-f]{8}$/);
		expect(document.name).toMatch(/^lucid_doc__page__banner__doc_[0-9a-f]{8}$/);
	});

	test("does not alter first three sections even if over limit without relation tail", () => {
		const raw = `lucid_doc__${"a".repeat(60)}__ver`;
		const res = toSafeTableName(raw, 63);
	...
	test("floors provided limit to minimum supported table-name limit", () => {
		...
		expect(withLowLimit.name).toMatch(
			/^lucid_doc__page__banner__rep_[0-9a-f]{8}$/,
		);
	});
});
