...
export default class ApiIntegrationsRepository extends StaticRepository<"lucid_api_integrations"> {
	constructor(db: KyselyDB, dbAdapter: DatabaseAdapter) {
		super(db, dbAdapter, "lucid_api_integrations");
	}
	...
	/**
	 * Selects an API integration and its scopes by ID.
	 */
	async selectSingleByIdWithScopes<V extends boolean = false>(
		...
		const query = this.db
			.selectFrom("lucid_api_integrations")
			.select((eb) => [
				"id",
				"key",
				"name",
				...
				this.dbAdapter
					.jsonArrayFrom(
						eb
							.selectFrom("lucid_api_integration_scopes")
							.select(["scope"])
							.whereRef(
								"lucid_api_integration_scopes.api_integration_id",
								"=",
								"lucid_api_integrations.id",
							),
					)
					.as("scopes"),
			])
			...
			.$call((qb) =>
				queryBuilder.tenantScope(qb, {
					...
					column: "lucid_api_integrations.tenant_key",
				}),
			);
	...
	/**
	 * Selects API integration credentials and scopes by public key.
	 */
	...
	/**
	 * Selects filtered API integrations with their scopes and total count.
	 */
							...
							customFilters: {
								scope: ({ eb, filter }) => {
									...
									return eb.exists(
										eb
											.selectFrom("lucid_api_integration_scopes")
											.select("lucid_api_integration_scopes.id")
											.whereRef(
												"lucid_api_integration_scopes.api_integration_id",
												"=",
												"lucid_api_integrations.id",
											...
											.where(
												"lucid_api_integration_scopes.scope",
												"in",
												values.map(String),
											),
									);
