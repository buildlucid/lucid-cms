...
const getMultipleController = factory.createHandlers(
	describeRoute({
		...
		parameters: openAPI.parameters({
			params: controllerSchemas.content.getMultiple.params,
			query: controllerSchemas.content.getMultiple.query.string,
			headers: {
				authorization: true,
			},
		}),
	...
	async (c) => {
		...
		const formattedQuery = await buildFormattedQuery(
			c,
			controllerSchemas.content.getMultiple.query.formatted,
		);

		const documents = await serviceWrapper(
			documentServices.content.getMultiple,
			{
				transaction: false,
				defaultError: {
					type: "basic",
