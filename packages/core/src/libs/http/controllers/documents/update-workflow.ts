...
const updateWorkflowController = factory.createHandlers(
	describeRoute({
		...
		requestBody: openAPI.requestBody(controllerSchemas.updateWorkflow.body),
	...
	async (c) => {
		...
		const workflow = await serviceWrapper(
			documentWorkflowServices.updateSingle,
			{
				...
				defaultError: {
					type: "basic",
					name: copy("server:core.routes.document.workflow.update.error.name"),
					message: copy(
						"server:core.routes.document.workflow.update.error.message",
					),
				},
			},
		)(context, {
