...
const createLogoUploadSessionController = factory.createHandlers(
	describeRoute({
		description: "Creates an upload session for an OAuth client logo.",
		tags: ["integrations"],
	...
	async (c) => {
		...
		const uploadRes = await serviceWrapper(
			oauthClientServices.createLogoUploadSession,
			{
				transaction: false,
				defaultError: {
					type: "basic",
...
export default createLogoUploadSessionController;
